...
data.map((item) => {
    kod += `
          <div class="w-[300px] h-[350px] border rounded-lg overflow-hidden cursor-pointer">
        <div class="flex border border-b-2">
            <img src="${item.flags.svg}" class="w-full h-[180px] object-cover" alt="">
        </div>
       <div class="p-6">
           <div>
            <h2 class="text-zinc-600 uppercase ">${item.region}</h2>
            <h1 class="my-2 font-bold hover:underline">${item.name}, <span class="italic">${item.capital}</span></h1>
           </div>
           <div class="flex justify-between">
            <span class="text-[14px]">Population: ${item.population}</span>
            <span class="text-[14px]">${item.area} km2</span>
           </div>
       </div>
    </div>
    `
...
function agakerim () {
    ...
    let selectedCountry = data[rndNum]
    let code =`
    <div class="md:m-auto md:w-[900px] md:h-[400px]  mt-6 md:flex md:gap-6 md:pt-[6rem]">
            <div class="md:w-[70%] h-full">
                <img src="${selectedCountry.flags.svg}" class="w-full h-[180px] md:h-full object-cover" alt="">
            </div>
           <div class="text-left">
           <h1 class="font-bold text-[1.2rem] md:text-[1.6rem] tracking-wide	mt-2">${selectedCountry.name}</h1>
            <h3 class="font-bold text-violet-600 underline mt-2">${selectedCountry.region}</h3>
            <p class="mt-2">Capital: ${selectedCountry.capital}</p>
            <p class="mt-2">Area: ${selectedCountry.area} km2</p>
            <p class="mt-2">Population: ${selectedCountry.population}</p>
           </div>
        </div>
`
randomCountry.innerHTML = code;
}
...
for(let i = 0; i < europe.length; i++){
        ...
        europe[i].addEventListener("click", () => {
            const euro  = data.filter(item => item.region === "Europe")
            show(euro)
...
for(let i = 0; i < asia.length; i++){
    asia[i].addEventListener("click", () => {
        const asia = data.filter(item => item.region === "Asia")
        show(asia)
        section.classList.add('hidden')
...
const americas = document.getElementsByClassName('americas')
for(let i = 0; i < americas.length; i++){
    americas[i].addEventListener("click", () => {
        const americas = data.filter(item => item.region === "Americas")
        show(americas)
        section.classList.add('hidden')
...
for(let i = 0; i < africa.length; i++){
    africa[i].addEventListener("click", () => {
...
for(let i = 0; i < oceania.length; i++){
    oceania[i].addEventListener("click", () => {
        const oceania = data.filter(item => item.region === "Oceania")
        show(oceania)
        section.classList.add('hidden')
    })
...
const antarctic = document.getElementsByClassName('antarctic')
for(let i = 0; i < antarctic.length; i++){
    antarctic[i].addEventListener("click", () => {
        const antarctic = data.filter(item => item.region === "Antarctic")
        show(antarctic)
        section.classList.add('hidden')
    })
...
const fa_star_of_life = document.querySelector('.fa-star-of-life')
fa_star_of_life.addEventListener("click" , () => {
    document.querySelector('body').classList.toggle('bg-slate-900')
...
for(let i = 0; i < countLink.length; i++){
    countLink[i].addEventListener("click" , () => {
        countLink[i].classList.add("text-violet-600")
        countLink[i].classList.add( 'border-violet-800') 
        for(let u = 0; u <  countLink.length; u++){
            if(countLink[u] != countLink[i]){
                countLink[u].classList.remove("text-violet-600")
                countLink[u].classList.remove( 'border-violet-800') 
...
const inpShow = document.querySelector('#inpShow')
const btnShowSearch = document.querySelector('#btnShowSearch')

btnShowSearch.addEventListener("click", () => {
    inpShow.classList.toggle("hidden")
    randomCountry.style.display = 'none';
    section.classList.add('md:h-[50vh]')
...
function findCountry(){
    let xod = ''
    const inpVal = inpSearch.value.toLowerCase()
    ...
    data.map((item) => {
        if(item.name.toLowerCase().includes(inpVal)){
            xod += `
            <div class="w-[300px] h-[350px] border rounded-lg overflow-hidden cursor-pointer">
            <div class="flex border border-b-2">
            <img src="${item.flags.svg}" class="w-full h-[180px] object-cover" alt="">
